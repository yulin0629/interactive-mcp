...
export const InteractiveInput: FC<InteractiveInputProps> = ({ question, questionId, predefinedOptions, onSubmit }) => {
  ...
  useInput((input, key) => {
    if ((key.upArrow || key.downArrow) && predefinedOptions?.length) {
      ...
      if (predefinedOptions && predefinedOptions.length > 0 && customValue === '') {
        ...
        valueToSubmit = customValue || (predefinedOptions && predefinedOptions[selectedIndex]) || '';
      }
  ...
  return (
    <>
      <Box flexDirection="column" marginBottom={1}>
        <Text bold color="cyan" wrap="wrap">{question}</Text>
      </Box>
